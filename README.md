...
It drives 55 weapons and 2034 finishes, 20 knives, 8 gloves and 94 glove finishes, 105 sticker
...
There are no catalogue files. Everything is parsed at load from CS2's own
`scripts/items/items_game.txt` and `resource/csgo_english.txt`, read through
`IFileManager.OpenFile` — Valve's own filesystem, so the VPKs are searched and nothing has to be
extracted first. It costs about 271 ms of one startup.

This replaced a pair of JSON files generated by a Python script at build time. The reason is not
elegance: a CS2 update silently invalidated them and nothing failed loudly enough to notice, so new
skins were simply absent until someone remembered to regenerate. Reading the game's own files means
the catalogue is whatever the installed game actually has.

[src/Services/SchemaCatalogue.cs](src/Services/SchemaCatalogue.cs) is a direct port of
[tools/build_catalog.py](tools/build_catalog.py) and [tools/build_gloves.py](tools/build_gloves.py),
which are kept as the reference for the parsing. Every trap they document still applies, because it
is the same file being read:

* `items_game.txt` contains **55 separate sticker_kits blocks** and several `paint_kits` blocks.
  Reading only the first gives 995 stickers instead of 11789.
* Localisation has to be matched per line. One regex over the whole of `csgo_english.txt` matches
  across entries.
* Twelve Doppler kits share a single `description_tag`, so the phase name has to come from the kit
  name rather than the tag.
* Gloves are not in the loot list format weapons use, so the weapon builder cannot see them at all.
  Each glove has **two** generations of paint kit name prefix, an original one and a later
  `glove_<type>_` scheme, and mixing them up silently offers finishes belonging to a different
  glove. Broken Fang is definition 4725, outside the 5027 to 5035 run, and its paints are prefixed
  `operation10_` rather than by the glove name. The mapping is checked against every paint kit whose
  material path lives under gloves, and all 94 are accounted for.
* Music kits are **not** item definitions. Only two `musickit` items exist in the schema. The kits
  live in `music_definitions` with their own ids, 1 to 101, and that id is what `inventory.MusicId`
  takes. Building the list from item definition indices instead produces 101 entries that all look
  right and all point at nothing.
...
Gloves, pins and music kits have no 3D preview, so they are shown as 2D icons. The art is CS2's own,
reached through the `s2r://` protocol, which resolves against the game's resource system rather than
against this addon:
...
`IFileManager.OpenDirectory` enumerates the 6804 econ images at load and
[tools/gen_econ_icons.py](tools/gen_econ_icons.py) writes them out as `ui/styles/econicons.css`.

This is worth stating because the docs here previously said econ art was unreachable from a
`custom_hud_layout`. That was drawn from `file://{images}` and `{resources}/images`, which do both
fail, and was never retested against `s2r://`.
...
src/Modules/Medals.cs          applies a saved pin
src/Modules/MusicKits.cs       applies a saved music kit
src/Modules/StickerSchemas.cs  sticker attribute names
src/Services/SchemaCatalogue.cs  the catalogue, built at load from the game's files
src/Services/GameFiles.cs      reads game files through Valve's filesystem
src/Services/ArsenalStore.cs   the storage seam the browser writes through
